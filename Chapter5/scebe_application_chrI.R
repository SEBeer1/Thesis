...
timer <- system.time({
  ...
  output <- foreach(i = 1:length(cpg.names), .combine = 'rbind', .export = 'scebe_function1') %dopar% {
  ...
  write.csv(output, file = paste0("/data2/sbeer/lmQTL/output/CpGSNPoutChr", chr, ".csv"), row.names=F)
