...
half_k_one_iteration = function(i, path.to.simFolder, path.to.outfolder, save.name = "onlyCSIteration"){
  ...
  phenoData = half_number_of_measurements(phenoData)
  ...
  gallop.out = gallop_function1_missing_data(phenoData, SNP.matrix, colnames(SNP.matrix), mod1 = mod1, k=3)
...
## drop waves 4, 6, 8
half_number_of_measurements = function(data){
  
  data = data %>% filter(!Wave %in% c(4,6,8))
  
  return(data)
}

pt.simFolder = "/depot/socgen/data/solomon_beer/uploadFolder/"
pt.outFolder = "/depot/socgen/data/solomon_beer/uploadFolder/sf5halfK/"
sn = "halfKIteration"
...
for(i in (10*starting.i-9):(10*starting.i)){
  print(i)
  half_k_one_iteration(i, pt.simFolder, pt.outFolder, save.name = sn)
}
