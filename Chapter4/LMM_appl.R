...
lmm_function1 = function(phenoData, SNP.matrix, SNP.names){
  ...
  phenoData = phenoData[,c(1,11,2,8,7,9)]
  colnames(phenoData) = c("ID", "Sample_Name", "Sex", "Time", "Height", "Weight")
  ...
  for(i in 1:ncol(SNP.matrix)){
    ...
    print(i)
    #print(phenoData$Sample_Name[1:5])
    #print(row.names(SNP.matrix)[1:5])

    j.df = left_join(phenoData, data.frame(SNP.matrix[,i], Sample_Name = row.names(SNP.matrix)), join_by(Sample_Name)) # this is updated on cluster
    #j.df = cbind(phenoData, SNP.matrix[,i])
    #print(head(j.df,2))
    colnames(j.df) = c("ID", "Sample_Name", "Sex", "Time", "Height", "Weight", "SNP")
    ...
    #out.lmm[i,] = c(coef(summary(mod1))[2,], 2*pnorm(-abs(coef(summary(mod1))[2,3])),coef(summary(mod1))[6,], 2*pnorm(-abs(coef(summary(mod1))[6,3])))
  }
  ...
  colnames(out.lmm) = c("CpG","cs.est", "cs.se", "cs.t", "cs.pval", "lon.est", "lon.se", "lon.t", "lon.pval", "method")
...
## cpgs for LMM
lmm.path = "/depot/socgen/data/solomon_beer/epiSCEBE/lmm.list.csv"
...
# read in CpG list for the LMM
cpgs = read.csv("lmm.list.csv")$x
...
# apply LMM: 
output = lmm_function1(input.data, CpG.matrix, cpg.names)
# output = data.frame(output) %>% drop_na()

# save output
# saveRDS(appl.list, file = save.name)
write.csv(output, "lmm.output.csv", row.names=T)
