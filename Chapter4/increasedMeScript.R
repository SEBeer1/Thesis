...
increased_me_one_iteration = function(i, path.to.simFolder, path.to.outfolder, save.name = "onlyCSIteration"){
  ...
  phenoData = add_measurement_error(phenoData, 3)
...
add_measurement_error = function(data, std){
  
  data = data %>% mutate(Weight = Weight + rnorm(nrow(data), sd = std))
  
  return(data)
}

pt.simFolder = "/depot/socgen/data/solomon_beer/uploadFolder/"
pt.outFolder = "/depot/socgen/data/solomon_beer/uploadFolder/sf4increaseMe/"
sn = "increaseMeIteration"

st = Sys.time()

for(i in (10*starting.i-1):(10*starting.i)){
  print(i)
  increased_me_one_iteration(i, pt.simFolder, pt.outFolder, save.name = sn)
}
