...
increasing_me_one_iteration = function(i, path.to.simFolder, path.to.outfolder, save.name = "onlyCSIteration"){
  ...
  phenoData = add_increasing_measurement_error(phenoData, 0.5, 1)
...
add_increasing_measurement_error = function(data, std, slope){
  
  data = data %>% mutate(Weight = Weight + slope*(Age.years-80/52)*rnorm(nrow(data), sd = std))
  
  return(data)
}

pt.simFolder = "/depot/socgen/data/solomon_beer/uploadFolder/"
pt.outFolder = "/depot/socgen/data/solomon_beer/uploadFolder/sf8increasingMe/"
sn = "increasingMeIteration"

st = Sys.time()

for(i in (10*starting.i-9):(10*starting.i)){
  print(i)
  increasing_me_one_iteration(i, pt.simFolder, pt.outFolder, save.name = sn)
}
