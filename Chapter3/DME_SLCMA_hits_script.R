...
maxt.tc = function(maxt){
  tryCatch(
    {temp = c(row.names(maxt$maxt$coefficients), maxt$maxt$coefficients[1,1], maxt$maxt$coefficients[1,3], maxt$maxt$coefficients[1,4], maxt$maxt$coefficients[1,2])
    return(temp)
    }, error = function(msg){
      return(c(NA, NA, NA, NA, NA))
    })
...
save.name = "/depot/socgen/data/solomon_beer/epigeneticDMEslcma/DME_SLCMA_sexphy_hits.rds"
list.hits = "/depot/socgen/data/solomon_beer/epigeneticDMEslcma/sexphy_hits.txt"
...
appl.list <- apply.gdsn(index.gdsn(f, "matrix"), margin=1,
                  ...
                  FUN=function(index, x)
                    apply_dme_slcma_DNAm_column(index, x,
